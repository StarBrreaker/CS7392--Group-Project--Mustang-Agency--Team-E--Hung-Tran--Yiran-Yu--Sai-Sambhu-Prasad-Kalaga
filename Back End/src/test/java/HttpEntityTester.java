...
public class HttpEntityTester {
    ...
    private static void registerProfessional(String preferredUsername, String firstName, String lastName,
        ...
        // Existing code to register a new user and professional remains unchanged

        // Convert the qualifications to JSON arrays
        String categoriesJson = new JSONArray(qualificationCategories).toString();
        String keywordsJson = new JSONArray(qualificationKeywords).toString();

        // New code to create a professional account request
        String accountRequestJson = String.format("{\"preferredUsername\":\"%s\", \"firstName\":\"%s\", \"lastName\":\"%s\", " +
                        "\"dob\":\"%s\", \"email\":\"%s\", \"phone\":\"%s\", \"mailingAddress\":\"%s\", " +
                        "\"degree\":\"%s\", \"institution\":\"%s\", \"dateOfAward\":\"%s\", " +
                        "\"qualificationCategories\":%s, \"qualificationKeywords\":%s}",
                preferredUsername, firstName, lastName, dob, email, phoneNumber, mailAddress, degree, institution, dateOfAward,
        ...
        HttpResponse<String> postResponse3 = client.send(postRequest3, BodyHandlers.ofString());
        System.out.println("Create Professional Account Request Response Status Code: " + postResponse3.statusCode());
        System.out.println("Create Professional Account Request Response Body: " + postResponse3.body());
    }
    ...
    private static void createProfessional(String requestDetails) throws Exception {
        ...
        JSONArray qualifications = obj.getJSONArray("qualifications");

        // Construct JSON for professional account creation, including qualifications
        JSONObject professionalRequestJson = new JSONObject();
        professionalRequestJson.put("username", username);
        professionalRequestJson.put("password", password);
        professionalRequestJson.put("firstName", firstName);
        professionalRequestJson.put("lastName", lastName);
        professionalRequestJson.put("dob", dob);
        professionalRequestJson.put("email", email);
        professionalRequestJson.put("phoneNumber", phoneNumber);
        professionalRequestJson.put("mailingAddress", mailAddress);
        professionalRequestJson.put("degree", degree);
        professionalRequestJson.put("institution", institution);
        professionalRequestJson.put("dateOfAward", dateOfAward);
        professionalRequestJson.put("qualifications", qualifications);
...
//                + mailAddress + "&degree=" + degree + "&institution=" + institution + "&dateOfAward=" + dateOfAward;
        HttpRequest postRequest1 = HttpRequest.newBuilder()
                ...
                .header("Content-Type", "application/x-www-form-urlencoded")
                .POST(BodyPublishers.ofString(professionalRequestJson.toString()))
                .build();
            ...
            switch (function) {
                ...
                case 17: { 
                    ...
                    if ("yes".equalsIgnoreCase(decision)) {
                        System.out.println("Enter the username associated with this deletion request to delete: ");
                        String username = sc.nextLine();
                        deleteProfessionalByUsername(username); // Assuming a similar method exists for employers
                        deleteUserByUsername(username);

...
let professional = localStorage.getItem('selectedProfessionalID'); // Retrieve the employer ID saved in localStorage
...
function fetchProfessionalAccountDeletetionRequestDetails() {
    fetch('http://localhost:8080/professionalAccountDeletionRequests/allPending')
    .then(response => {
        if (!response.ok) {
    ...
    .then(data => {
        let tableBody = document.getElementById("professionalDeleteRequestTable").getElementsByTagName('tbody')[0];
...
function updateStatusThenDeactivate(username, requestId) {
    ...
    fetch(`http://localhost:8080/professionalAccountDeletionRequests/${requestId}`, {
    method: 'PUT',
    headers: {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    fetchProfessionalAccountDeletetionRequestDetails();
